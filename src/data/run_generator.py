...
def run_suite(config_path, output_dir, days=100):
    ...
    for archetype_name, archetype_data in config['archetypes'].items():
        ...
        # Extract generation parameters from the 'config' block
        ...
        # Convert JSON string keys (e.g., "0.33") back to floats
        processed_persona = {}
...
if __name__ == "__main__":
    ...
    json_files = current_file_dir.glob("*.json")
    
    # Loop through each file and run the suite
    for suite_file in json_files:
        print(f"Running suite: {suite_file.name}")
        run_suite(suite_file, dataset_path, days=10000)
